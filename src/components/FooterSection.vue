<template>
  <footer class="border-t border-etrib-teal/20 py-8 md:py-10 bg-paper">
    <div class="max-w-6xl mx-auto px-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10 mb-10">
        ...
        <!-- Links -->
        <div class="text-center md:text-left">
          <div class="font-display tracking-wider text-etrib-teal mb-4">NAVEGACIÓN</div>
          <ul class="space-y-2">
            <li v-for="link in footerLinks" :key="link.href">
              <a :href="link.href" class="text-sm text-ink/50 hover:text-etrib-teal transition-colors">
                {{ link.label }}
              </a>
            </li>
          </ul>
